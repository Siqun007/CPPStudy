...
namespace Render {
    ...
    public:
        ...
        // 判断某个 param 是否存在
        // 按关键词筛选 param
        void SetParam(const std::string& param_name, const float& param);
        void GetParamValue(const std::string& param_name, float& out_value) const;
        void DeleteParam(const std::string& param_name);
        bool HasParam(const std::string& param_name) const;
        void FindParamByKeyWord(const std::string& key_word) const;
